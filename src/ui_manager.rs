...
impl UiManager {
    ...
    fn is_usable_cover_source_path(path: &Path) -> bool {
        path.exists() && Self::is_valid_image_file(path)
    }
        ...
        thread::spawn(move || {
            ...
            while let Ok(request) = cover_art_lookup_rx.recv() {
                ...
                let _ = cover_art_bus_sender.send(protocol::Message::Playback(
                    protocol::PlaybackMessage::CoverArtChanged {
    ...
    fn update_cover_art(&mut self, track_path: Option<&PathBuf>) {
        ...
        self.next_cover_art_lookup_request_id = self
            .next_cover_art_lookup_request_id
            .saturating_add(1)
            .max(1);
        self.pending_cover_art_lookup_request_id = self.next_cover_art_lookup_request_id;
        self.pending_cover_art_lookup_track_path = requested_track_path.clone();
        let _ = self.cover_art_lookup_tx.send(CoverArtLookupRequest {
            request_id: self.pending_cover_art_lookup_request_id,
    ...
    fn resolve_track_cover_art_path(&mut self, source_index: usize) -> Option<PathBuf> {
        ...
        if let Some(Some(existing_path)) = self.track_cover_art_paths.get(source_index) {
            if Self::failed_cover_path(existing_path)
                || !Self::is_usable_cover_source_path(existing_path.as_path())
            {
                Self::delete_cached_image_file_if_managed(existing_path.as_path());
    ...
    fn load_track_row_cover_art(path: Option<&PathBuf>) -> (Image, bool) {
        let image = path.and_then(|path| {
            Self::try_load_cover_art_image_with_kind(path, protocol::UiImageKind::CoverArt)
        });
        let has_album_art = image.is_some();
        (image.unwrap_or_default(), has_album_art)
    }
    ...
    fn canonical_local_track_path(path: &Path) -> Option<PathBuf> {
        if is_remote_track_path(path) {
            return None;
        }
    ...
    fn metadata_link_path_matches_track_path(
        track_path: &Path,
        pending_path: &Path,
        pending_canonical: Option<&PathBuf>,
    ) -> bool {
        if Self::is_equivalent_track_path(track_path, pending_path) {
            return true;
        }
        let Some(pending_canonical) = pending_canonical else {
            return false;
        };
        Self::canonical_local_track_path(track_path).as_ref() == Some(pending_canonical)
    }

    fn metadata_link_title_matches_track_title(track_title: &str, pending_title: &str) -> bool {
        track_title.trim().to_lowercase() == pending_title.trim().to_lowercase()
    }
    ...
    fn set_playing_track(
        ...
        if let Some(path) = path.as_ref() {
            let resolved_album_artist = self
                .playing_track
                .metadata
                .as_ref()
                .is_some_and(|metadata| metadata.album_artist.trim().is_empty())
                .then(|| self.resolve_album_artist_for_track_path(path.as_path()));
            if let Some(metadata) = self.playing_track.metadata.as_mut() {
                if metadata.album_artist.trim().is_empty() {
                    if let Some(album_artist) = resolved_album_artist {
                        metadata.album_artist = album_artist;
                    }
                }
                if metadata.album_artist.trim().is_empty() {
                    metadata.album_artist = metadata.artist.trim().to_string();
                }
            }
        }
        path_changed
    ...
    fn refresh_playing_track_metadata(&mut self) -> bool {
        ...
        let mut resolved = self.resolve_metadata_for_track_path(path.as_path());
        if let Some(metadata) = resolved.as_mut() {
            if metadata.album_artist.trim().is_empty() {
                metadata.album_artist = self.resolve_album_artist_for_track_path(path.as_path());
            }
            if metadata.album_artist.trim().is_empty() {
                metadata.album_artist = metadata.artist.trim().to_string();
            }
    ...
    fn viewer_template_metadata_for_display_target(
        ...
        display_metadata.map(|metadata| {
            let fallback_album_artist = if metadata.album_artist.trim().is_empty() {
            ...
            .is_some_and(|playing_path| Self::is_equivalent_track_path(playing_path, path))
        {
            ...
            if self.playing_track.metadata.as_ref().map(|meta| {
                meta.title == detailed.title
                    ...
                    && meta.album_artist == detailed.album_artist
    ...
    fn refresh_visible_playlist_cover_art_rows(&mut self) -> bool {
        ...
        let _ = self.ui.upgrade_in_event_loop(move |ui| {
            ...
            for (view_row, thumbnail_path) in updates {
                ...
                if let Some(image) = UiManager::try_load_cover_art_image_with_kind(
                    thumbnail_path.as_path(),
                    protocol::UiImageKind::CoverArt,
                ) {
                    row_data.album_art = image;
                    row_data.has_album_art = true;
                } else {
    ...
    fn resolve_library_cover_art_path(&mut self, track_path: &PathBuf) -> Option<PathBuf> {
        if let Some(cached) = self.library_cover_art_paths.get(track_path).cloned() {
            if let Some(path) = cached.clone() {
                if Self::failed_cover_path(&path) || !Self::is_usable_cover_source_path(&path) {
                    Self::delete_cached_image_file_if_managed(path.as_path());
                    self.library_cover_art_paths
    ...
    fn select_library_track_for_pending_metadata_link(&mut self) {
        ...
        let pending_title = self
            ...
            .filter(|title| !title.is_empty());
        let pending_canonical_path = pending_path
            .as_deref()
            .and_then(Self::canonical_local_track_path);
        ...
        let selected_index = self
            ...
            .find_map(|(index, entry)| {
                ...
                if pending_path.as_ref().is_some_and(|path| {
                    Self::metadata_link_path_matches_track_path(
                        track.path.as_path(),
                        path.as_path(),
                        pending_canonical_path.as_ref(),
                    )
                }) {
                    return Some(index);
                }
                pending_title
                    .as_ref()
                    .is_some_and(|title| {
                        Self::metadata_link_title_matches_track_title(&track.title, title)
                    })
                    .then_some(index)
            });
    ...
    fn activate_metadata_link(
        ...
        let mut pending_track_path: Option<PathBuf> = None;
        let mut pending_track_title: Option<String> = None;
        let (target_view, fallback_root) = match link.kind {
            ...
            protocol::MetadataLinkKind::Title => {
                ...
                pending_track_path = link.track_path.clone();
                pending_track_title = Some(value.clone());
                (
                    LibraryViewState::AlbumDetail {
                        album,
                        album_artist,
        ...
        if reset_stack_to_root {
            self.navigate_to_library_view_from_root(fallback_root.clone(), target_view.clone());
        } else {
            self.navigate_to_library_view(target_view.clone());
        }
        self.pending_metadata_link_track_path = pending_track_path;
        self.pending_metadata_link_track_title = pending_track_title;
                ...
                Ok(message) => {
                    ...
                    match message {
                        ...
                        protocol::Message::Playback(
                            ...
                            if request_id != self.pending_cover_art_lookup_request_id
                                || requested_track_path != self.pending_cover_art_lookup_track_path
                            {
                                trace!(
                                    "Ignoring stale cover art response id={} requested={:?} pending_id={} pending={:?}",
                                    request_id,
                                    requested_track_path,
                                    self.pending_cover_art_lookup_request_id,
                                    self.pending_cover_art_lookup_track_path
                                );
                                continue;
                            }
                            if cover_art_path.is_some() {
