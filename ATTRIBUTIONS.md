# Attribution

This project contains third-party code with the following licenses.

## Color Schemes

### Solarized
- **Schemes**: `solarized_dark`, `solarized_light`
- **Source**: https://github.com/altercation/vim-colors-solarized
- **License**: MIT License
- **Location**: `config/color_schemes.toml`

### Dracula
- **Schemes**: `dracula_dark`
- **Source**: https://github.com/dracula/dracula-theme
- **License**: MIT License
- **Location**: `config/color_schemes.toml`

### Nord
- **Schemes**: `nord_dark`
- **Source**: https://github.com/nordtheme/nord
- **License**: MIT License
- **Location**: `config/color_schemes.toml`

### Gruvbox
- **Schemes**: `gruvbox_dark`, `gruvbox_light`
- **Source**: https://github.com/morhetz/gruvbox
- **License**: MIT License
- **Location**: `config/color_schemes.toml`

### Catppuccin
- **Schemes**: `catppuccin_mocha`, `catppuccin_latte`
- **Source**: https://github.com/catppuccin/palette
- **License**: MIT License
- **Location**: `config/color_schemes.toml`

### Tokyo Night
- **Schemes**: `tokyonight_night`, `tokyonight_day`
- **Source**: https://github.com/folke/tokyonight.nvim
- **License**: Apache-2.0
- **Location**: `config/color_schemes.toml`

## Icons

### Tabler Icons
- **Source**: https://tabler.io/icons
- **Upstream Repository**: https://github.com/tabler/tabler-icons
- **License**: MIT License
- **Copyright**: Copyright (c) 2020-2026 Paweł Kuna
- **Location**: `src/ui/icons/tabler/`
- **Policy**: All icons in `src/ui/icons/tabler/` must be fetched from the official Tabler GitHub repository; do not create custom local SVG variants.

### OpenSubsonic Logo
- **Source**: https://github.com/opensubsonic/open-subsonic-api/blob/main/assets/icons/logo.svg
- **License**: Apache-2.0
- **Copyright**: OpenSubsonic contributors
- **Location**: `src/ui/icons/opensubsonic/logo.svg`

## Fonts

No custom fonts are bundled with this project.

## Other Assets

No additional third-party assets are included beyond those listed above.

---

For the full license texts of bundled components, please refer to the individual license files in their respective directories.
